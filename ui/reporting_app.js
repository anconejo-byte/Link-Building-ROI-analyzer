/*
  ...
  This script wires the visible CSV import controls and mock-data loaders.
  Key handlers to verify after conflict resolution:
  - handleCsvUpload
  - loadMockData
  - addHandlers bindings for uploadCsvBtn/uploadCsvBtnTop and loadMockData buttons
*/
const state = {
  operations_period: [],
  links: [],
...
function addHandlers() {
  ...
  document.getElementById("loadMockDataBtnTop").addEventListener("click", loadMockData);
  document.getElementById("loadMockDataBtnInline").addEventListener("click", loadMockData);
  ...
  document.getElementById("linksCsvInput").addEventListener("change", (event) => setPendingCsvFile(event.target.files?.[0] || null));
  document.getElementById("linksCsvInputTop").addEventListener("change", (event) => setPendingCsvFile(event.target.files?.[0] || null));
  document.getElementById("uploadCsvBtn").addEventListener("click", handleCsvUpload);
  document.getElementById("uploadCsvBtnTop").addEventListener("click", handleCsvUpload);
...
function handleCsvUpload() {
  ...
  reader.onload = () => {
    try {
      ...
      document.getElementById("linksCsvInputTop").value = "";
