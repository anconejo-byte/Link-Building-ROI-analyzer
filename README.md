...
3. Add links manually and/or upload `ui/mock_links.csv` (or your own CSV with required columns).
   - CSV is available in the **CSV Import (Here)** highlighted section near the top of the page.
...
If a merge conflict resolves with a suspiciously empty diff, verify these anchors still exist:
- `ui/reporting_app.html`: `CSV Import (Here)`, `Upload CSV`, `Use Built-in Mock Data`
- `ui/reporting_app.js`: `handleCsvUpload`, `loadMockData`, and top-of-file conflict-resolution note
